...
export type Vec<T> = T[];
`;function wt(){return g({template:Et})}var Rt=`{{header}}
...
{{/each}}
`;function xt(s){let r=s.abis.map(({name:e})=>e);return g({template:Rt,data:{abiCapitalizedNames:r}})}var pt=class{constructor(r){let{abiFiles:t,outputDir:e}=r;this.files=[],this.outputDir=e,this.abiFiles=t,this.abis=this.abiFiles.map(i=>new B({filepath:i.path,rawContents:JSON.parse(i.contents),outputDir:e})),this.assembleAllFiles()}assembleAllFiles(){let r=this.abis.find(e=>e.commonTypesInUse.length>0);this.abis.forEach(e=>{let i={path:e.dtsFilepath,contents:e.getDtsDeclaration()},p={path:e.factoryFilepath,contents:e.getFactoryDeclaration()};this.files.push(i),this.files.push(p)});let t={path:`${this.outputDir}/index.ts`,contents:xt({abis:this.abis})};if(this.files.push(t),r){let i={path:(0,Ft.join)(this.outputDir,"common.d.ts"),contents:wt()};this.files.push(i)}}};0&&(module.exports={AbiTypeGen});
